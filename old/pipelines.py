...
class OldPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `old`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM old WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO old (date, title, link, content)"
